...
final class InsuranceXmlCommandLineTest extends TestCase
{
    private GenerateInsuranceXmlService $insuranceXmlService;

    protected function setUp(): void
    {
        parent::setUp();
        $this->insuranceXmlService = new GenerateInsuranceXmlService();
    }

    private function getValidDto(): GenerateInsuranceXMLDTO
    {
        return new GenerateInsuranceXMLDTO([
            'car_brand' => 'Toyota',
            'car_fuel' => 'Gasolina',
            'car_model' => 'Corolla',
            'car_power' => '132',
    ...
    public function testGenerateXmlWithValidData()
    {
        ...
        $xmlString = $this->insuranceXmlService->__invoke($dto);
        $xml = new SimpleXMLElement($xmlString);

        $this->assertContains((string) $xml->Datos->DatosAseguradora->SeguroEnVigor, ['S', 'N']);
        $this->assertContains((string) $xml->Datos->DatosGenerales->CondPpalEsTomador, ['S', 'N']);
        $this->assertContains((string) $xml->Datos->DatosGenerales->ConductorUnico, ['S', 'N']);
    ...
    public function testGenerateInsuranceXmlCommandWithMissingFields()
    {
        $data = [
            'car_brand' => 'Toyota',
            'car_fuel' => 'Gasolina',
